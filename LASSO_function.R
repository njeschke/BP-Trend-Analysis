...
#Function that runs a LASSO regression on a dataframe

LASSO_funct <- function(df, seed = 123) {
  ...
  # 6. Fit LASSO logistic regression
  lasso_fit <- cv.glmnet(x_train, y_train, family = "binomial", alpha = 1)
  best_lambda <- lasso_fit$lambda.min
  
  # 7. Predict on test set
  pred_probs_test <- predict(lasso_fit, newx = x_test, s = best_lambda, type = "response")
  pred_class_test <- ifelse(pred_probs_test > 0.5, 1, 0)
  
  # 8. Output confusion matrix and fitted model
...
#Function that plots the top 20 predictors resulting from a LASSO 

LASSO_plot_funct <- function(lasso_fit, best_lambda, top_n = 2, title = NULL) {
  ...
  ggplot(coef_df, aes(x = reorder(variable, abs_coef), y = coefficient)) +
    ...
    labs(
      title = ifelse(is.null(title),
                     paste("Top", top_n, "Lasso Predictors"),
                     title),
      x = "Predictor",
      y = "Coefficient"
    ) +
